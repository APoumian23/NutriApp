...
export default function GastoEnergetico() {
  ...
  const {resultadoMifflinGeb, resultadoMifflinGet} = useAppSelector(
    state => state.main,
  );
  const dispatch = useAppDispatch();

  function calculatorMifflinHandler() {
    ...
    dispatch(gebMifflinCalc(data));
    dispatch(getMifflinCalc(data));
  }
    ...
    <SafeAreaView flex={1} gap={50} justifyContent="center">
      <ScrollView>
        <VStack justifyContent="center" gap={15} mx={20}>
          <Card gap={5}>
            <Heading>Gasto Energético por Fórmula Mifflin</Heading>
            <Text>Sexo:</Text>
            <UISelector
              items={[
            ...
            <UIButton
              title="Calcular"
              bg=""
              onPress={calculatorMifflinHandler}
            />
            <Text>Resultado</Text>
            <Text>
              {resultadoMifflinGeb &&
                `El resultado GEB es ${resultadoMifflinGeb} calorias 
El resultado GET es ${resultadoMifflinGet} calorias `}
            </Text>
